    ...
    return {
        getSkeleton: function (id) {
            ...
            return {
                ...
                sync: {
                    updateCounter: 0
                    ,lastTimeUpdated: (new Date()).toJSON()
                },
        ...
        , initWithData: function (userscript_initial_data) {
            if (typeof userscript_initial_data === "object") {
                ...
                var self = {
                    ...
                    , increaseUpdateCounterAndLastTimeUpdated : function(){
                        // Zähler für ein neues Update erhöhen
                        userscript_data.sync.updateCounter = userscript_data.sync.updateCounter + 1;
                        // Zeitstempel aktualisieren
                        userscript_data.sync.lastTimeUpdated = (new Date()).toJSON();
                        
                        return self;
                    }
                    , getUpdateCounter : function(){
                        return userscript_data.sync.updateCounter;
                    }
                    , getLastTimeUpdated : function(){
                        return userscript_data.sync.lastTimeUpdated;
                    }
                    , getLastTimeUpdated_asDate : function(){
                        return new Date(self.getLastTimeUpdated());
                    }
                    , save: function (no_update) {
                        if(typeof no_update === "undefined"){
                            // Zähler für ein neues Update erhöhen
                            self.increaseUpdateCounterAndLastTimeUpdated();
                        }
