...
//                                contentScriptOptions: userscript_entry.options || {} 
			...
			if (script_settings["include-jquery"] === "true" || script_settings["include-jquery"] === true) {
				page_injection_object.contentScriptFile.push(resource_path + "libs/jquery/jquery-2.2.3.min.js");
			}
...
//								,"64": "./icon/icon-64.png"
	...
	, check_for_userscript_settings_and_save_it: function (userscript, worker, override_userscript_id, charset, moreinformations) {
		...
		if(us.error_code === 101 && !basic_helper.empty(alternative_name)){
			...
			// @todo
			// übler Workaround, da die Events von AngularJS nicht direkt aufgerufen werden können ...
			worker.port.emit("USI-FRONTEND:Event-Wrapper", {event_name: "USI-FRONTEND:EditUserscipt_newFromFile", data: modified_userscript});
			worker.port.emit("USI-FRONTEND:Event-Wrapper", {event_name: "USI-FRONTEND:changeTab", data: "createOrEdit"});
			...
			if (override_userscript_id === null || !basic_helper.isset(override_userscript_id)) {
				...
				for (var i in as) {					
					if (as[i].settings.name == us.name // Namen sind identisch
						...
						worker.port.emit("USI-BACKEND:same-userscript-was-found", {id: i, userscript: userscript, moreinformations: moreinformations});
			...
			if (old_userscript_was_found === 0) {
				...
				if(!basic_helper.empty(us.resource)){
					var resource_allowed_types = parse_userscript.get_userscript_keyword_config_by_name("resource");
					
					var one_resource, resource_name, resource_url, resource_charset;
					var resource_cache = [];
					var this_module = this;
					var resource_count = us.resource.length;
					
					for(var j in us.resource){
						// in [0] => name , [1] => url
						one_resource	=	us.resource[j].split(/\s+/);
						...
						resource_url = basic_helper.typeGuess(resource_url, resource_allowed_types.types);
						
						if(/^http/.test(resource_url)){
							
							(function(c){
							
								this_module.load_image_and_convert_to_datauri_or_return_text(c.resource_url, function (data, response_contenttype) {
									// Icon in der extra Variable sichern!
									resource_cache.push({name: c.resource_name, data: data, mime_type: response_contenttype, origUrl: c.resource_url});
																		
									// Überprüfung auf fehlerhafte Zeichen
									if(!/^image/.test(response_contenttype) && 
											(data.length !== data.replace(/\uFFFD/g, "").length)){
										// Wenn es kein Bild ist, und fehlerhafte Zeichen gefunden werden, sende eine Warnung heraus
										worker.port.emit("USI-BACKEND:get-alert", lang.error_userscript_settings + lang.found_bad_encoding);
									}

									if (resource_count === (parseInt(c.j) + 1)) {
										// Resourcen sichern
										us.resources_data = resource_cache;

										// speichere dieses Skript
										this_module.save_in_storage(c.id,
												{id: c.id,
													userscript: c.userscript,
													settings: c.us,
													deactivated: false,
													moreinformations: c.moreinformations}
										);
									}
								}, c.resource_charset);
							
							// Dies wird direkt ausgeführt und der Variable "c" übergeben, damit das Closure Problem umgegangen werden kann
								...
								resource_charset: resource_charset}));
						}
					}
				}
				...
				// Verarbeite das @icon
				if(!basic_helper.empty(us.icon)){
					var icon_allowed_types = parse_userscript.get_userscript_keyword_config_by_name("icon");
					
					// icon überschreiben! und setzt es auf Null falls nicht genutzt werden kann
					us.icon = basic_helper.typeGuess(us.icon, icon_allowed_types.types);
					
					// Falls es eine externe HTTP(S) Datei ist lade sie herunter
					if(/^http/.test(us.icon)){
						
						this.load_image_and_convert_to_datauri_or_return_text(us.icon, function(datauri){
							// Icon in der extra Variable sichern!
							us.icon_data = datauri;

							// speichere dieses Skript
							this.save_in_storage(id,
								{id: id,
									userscript: userscript,
									settings: us,
									deactivated: false,
					...
					for (var require_index in us.require) {
						...
						if (require_address !== null) {
							...
							(function(c){
								...
								this_module.__load_external_by_url(c.require_address, function (require_result) {

									// erstmal im Cache sichern
									require_cache.push({url: c.one_require, text: require_result});

									// wenn es der letzte Durchlauf ist!
									if ((parseInt(c.require_index) + 1) === require_length) {

										// speichere dieses Skript sobald alle require Statements erfolgreich durchlaufen sind!
										this_module.save_in_storage(
												c.id,
												{id: c.id,
													userscript: c.userscript,
													require_scripts: require_cache,
													settings: c.us,
													deactivated: false,
													moreinformations: c.moreinformations}
										);
...
//					id = override_userscript_id = us = null;
					...
					worker.port.emit("USI-BACKEND:get-alert", lang.userscript_was_saved + " -> ID " + id);
	...
	, __load_external_by_url: function(url_str, callback, charset){
...
//				charset = "cp1252";
	...
	, load_userscript_by_url: function (url_str, callback, charset) {
		...
		if (/.*\.user([()\d]*)\.js$/.test(url_str)) {
			return this.__load_external_by_url(url_str, callback, charset);
		} else {
			return false;
		}
	}
	 ...
	 * @returns {void}
	 */
	, save_in_storage: function (key, value) {
		// Den Wert speichern!
