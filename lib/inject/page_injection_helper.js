...
var page_injection_helper = {
	...
	, prepare_includes_and_excludes: function (rules, script_settings) {
		...
		for (var j in rules) {
			...
			var script_url_rule = rules[j];

			if (typeof script_url_rule === "string" && typeof script_url_rule.trim === "function") {
				// Entferne die überflüssigen Leerzeichen
				script_url_rule = script_url_rule.trim();
			}
			 ...
			 *******************************************/
			if (script_url_rule.indexOf("*") === -1) {
				// kein Stern gefunden, nimm es als String
				
				// Setze einfach nur den String ins Array!
				if(url_c.isValidURI(script_url_rule) === true){
					result.push(script_url_rule);
				}
			} else {
				...
				if (script_settings["clean-include"] !== "true") {
					...
					var test_url = parse_userscript.replace_wildcards_in_url(script_url_rule);
	...
	, add_userscript_to_page: function (userscript_entry) {
		...
		if (!basic_helper.empty(userscript_entry.settings.include) &&
			...
			if (script_settings.include.length > 0) {
				...
				var result_includes = this.prepare_includes_and_excludes(script_settings.include, script_settings);
			...
			if (typeof exclude_rules !== "undefined" && exclude_rules.length > 0) {
				...
				var prepared_result_excludes = this.prepare_includes_and_excludes(exclude_rules, script_settings);
			...
			if (script_settings["use-greasemonkey"] === "true" || script_settings["use-greasemonkey"] === true) {
				...
				page_injection_object.onAttach = function (worker) {
					...
					worker.port.on("USI-BACKEND:GM_openInTab", function (value_pair) {
						...
						var url_helper = require("sdk/url");

						// Prüft ob value_pair.url wirklich valide ist!
						if (url_helper.isValidURI(value_pair.url) === true) {
