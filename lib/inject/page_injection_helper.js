...
//                                contentScriptOptions: userscript_entry.options || {} 
			...
			if (script_settings["include-jquery"] === "true" || script_settings["include-jquery"] === true) {
				page_injection_object.contentScriptFile.push(resource_path + "libs/jquery-2.2.3.min.js");
			}
			...
			if (preferences.prefs.options_always_activate_greasemonkey === true || 
				...
				page_injection_object.contentScriptOptions.scriptMetaStr	=	parse_userscript.find_lines_with_settings(contentScript_result).join("\n");
...
//								,"64": "./icon/icon-64.png"
	 ...
	 ************************************************************************/
	  ...
	  * @returns {void}
	  */
	 , load_image_and_convert_to_datauri : function(url, callback){
			...
			xhr.onload = function () {
				...
				if (this.status === 200) {
					...
					if (response_contenttype === null || basic_helper.empty(response_contenttype)) {
						...
						response_contenttype = "image/png";
					}
					...
					if (!basic_helper.empty(raw)) {
						...
						callback("data:" + response_contenttype + ";base64," + base64.encode(raw));
		...
		if (basic_helper.isset(us.error_message)) {
			...
			if (old_userscript_was_found === 0) {
				...
				if(!basic_helper.empty(us.resource)){
					...
					var one_resource, resource_name, resource_url;
					var resource_cache = [];
					...
					for(var j in us.resource){
						...
						if(/^http/.test(resource_url)){
							...
							(function(c){
								...
								this_module.load_image_and_convert_to_datauri(c.resource_url, function (datauri) {
									// Icon in der extra Variable sichern!
									resource_cache.push({name: c.resource_name, data: datauri});
									...
									if (resource_count === (parseInt(c.j) + 1)) {
										...
										this_module.speichere_in_storage(c.id,
												{id: c.id,
				...
				if(!basic_helper.empty(us.icon)){
					...
					if(/^http/.test(us.icon)){
						...
						this.load_image_and_convert_to_datauri(us.icon, function(datauri){
							// Icon in der extra Variable sichern!
							us.icon_data = datauri;

							// speichere dieses Skript
							this.speichere_in_storage(id,
								{id: id,
									userscript: userscript,
									settings: us,
									deactivated: false,
					...
					for (var require_index in us.require) {
						...
						if (require_address !== null) {
							...
							(function(c){
								...
								this.__load_external_by_url(c.require_address, function (require_result) {
									...
									if ((parseInt(c.require_index) + 1) === require_length) {
										...
										this_module.speichere_in_storage(
												c.id,
												{id: c.id,
													userscript: c.userscript,
...
//					id = override_userscript_id = us = null;
	...
	, __load_external_by_url: function(url_str, callback, charset, notOverrideMimeType){
...
//				charset = "cp1252";
			...
			try{
				...
				// wichtig für Nicht Text Rückgaben!
				if(notOverrideMimeType === true){
					delete request_options.overrideMimeType;
				}
