	...
	if (!basic_helper.empty(userscript_entry.settings.include) && 
		...
		if (script_settings.include.length > 0) {
			...
			for (var j in script_settings.include) {
				...
				if(typeof script_include_url === "string" && typeof script_include_url.trim === "function"){
					// Entferne die überflüssigen Leerzeichen
					script_include_url = script_include_url.trim();
				}
		...
		var page_injection_object = {
				...
				exclude: /resource:\/\/firefox-addon-usi-at-jetpack\/.*/,
