...
userscript_storage = require("../storage/userscript").userscript_storage;
...
var page_injection_helper = {
	...
	re_init_page_injection: function () {
		...
		for (var i in all_userscripts) {
			...
			this.add_userscript_to_page(all_userscripts[i]);
		...
		if(us.error_code === 101 && !basic_helper.empty(alternative_name)){
			...
			if (override_userscript_id === null || !basic_helper.isset(override_userscript_id)) {
				...
				if (old_userscript_was_found === 0) {
					...
					return this.update_userscript(userscript_handle, userscript, us, moreinformations, worker);
          ...
          userscript_handle.
            ...
            if (!basic_helper.empty(settings.resource)) {
                ...
                for (var j in settings.resource) {
                    ...
                    // Resource nachladen!
                    userscript_handle.loadAndAddExternals("resource", resource_url, resource_name, resource_charset);
            ...
            if (!basic_helper.empty(settings.icon)) {
                ...
                // icon URL
                var icon_url = basic_helper.typeGuess(settings.icon, icon_allowed_types.types);

                // icon nachladen
                userscript_handle.loadAndAddExternals("icon", icon_url);
            ...
            if (!basic_helper.empty(settings.require)) {
                ...
                for (var require_index in settings.require) {
                    ...
                    // Überprüfe die URL
                    require_url = basic_helper.typeGuess(one_require, require_allowed_types.types);

                    // Nachladen des benötigten Skripts
                    userscript_handle.loadAndAddExternals("require", require_url);

                }
            }
            // füge das Skript gleich hinzu, damit es ausgeführt werden kann!
            this.add_userscript_to_page(userscript_handle.getData());
