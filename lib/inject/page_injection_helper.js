...
var page_injection_helper = (function () {
    ...
    var self = {
         ...
         * @returns {Boolean}
         */
        , userscriptInjection_onUpdate: function (details) {
            ...
            if (details.url) {
                ...
                return self.checkUserscriptInjection(details.tabId, details.url, details.transitionType);
            } else {
                // nichts zu tun
                return false;
            }
         ...
         * @returns {Boolean}
         */
        , checkUserscriptInjection: function (tabId, tabUrl, transitionType) {
        ...
        , re_init_page_injection: function () {
            ...
            userscript_storage.then(async (storage) => {
                ...
                const all_userscripts = storage.getAll();
                ...
                if (all_userscripts.length > 0) {
                     ...
                     */
                    if(!browser.webNavigation.onCommitted.hasListener(self.userscriptInjection_onUpdate)){
                        browser.webNavigation.onCommitted.addListener(self.userscriptInjection_onUpdate);
                    }
        ...
        , get_rules_and_exec_object: async function (userscript_handle) {
            ...
            if(userscript_handle.getSettings().spa === true){
                // SPA Userscripte
                return await self.create_spa_userscript_exec_object(userscript_handle);
            }else{
                // Normale Userscripte
                return await self.create_normal_userscript_exec_object(userscript_handle);
            }
         ...
         */
        , create_spa_userscript_exec_object : async function (userscript_handle){
            
            const script_settings = userscript_handle.getSettings(),
            userscript_id = userscript_handle.getId(),
            original_userscript = self.add_require_scripts(userscript_handle);
            ...
            const gm = await self.add_GM_Functions(userscript_handle);

            let contentScript = "";
            // Wenn jQuery gefordert ist 
            if (typeof script_settings["include-jquery"] !== "undefined") {
                contentScript += await self.add_jQuery_Functions(script_settings["include-jquery"]);
            }
            
            // letztendlich die Require Skripte und das Userscript anhängen
            contentScript += self.add_require_scripts(userscript_handle);
        ...
        , create_normal_userscript_exec_object : async function (userscript_handle){
            ...
            if (typeof script_settings["include-jquery"] !== "undefined") {
                contentScript += await add_jQuery_Functions(script_settings["include-jquery"]);
            }
            ...
            if (config_data.greasemonkey.global_active === true ||
                ...
                gm = await self.add_GM_Functions(userscript_handle);
         ...
         * @returns {string}
         */
        , add_require_scripts: function (userscript_handle) {
            // sammelt alle Skripte die auf der Seite eingebunden werden sollen
            let contentScript = new Array();
         ...
         * @returns {String}
         */
        , add_jQuery_Functions : async function(include_jQuery){
         ...
         * @returns {object}
         */
        , add_GM_Functions: async function (userscript_handle) {
