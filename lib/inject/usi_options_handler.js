...
// Sync Verwaltung            
    sync_handler = require("lib/storage/sync").sync_handler,
// Bevorzugte Sprachen
...
//		,baseurl: chrome_path
	...
	,onAttach: function (worker) {
		...
		worker.port.on("USI-BACKEND:sync-test-login", function () {
			sync_handler.testAccess(function(success_response){
				// Todo
		...
		// Sync Server --- neuen Benutzer erstellen
		worker.port.on("USI-BACKEND:sync-create-new-account", function (settings) {
            ...
            // Server URL Setzen
            sync_handler.setServerUrl(settings.url);

			// Neuen Benutzer erstellen
            sync_handler.createUser(null, function(success_response){
				
				var text = "Benutzer wurde erstellt -> ID ( " + success_response.data.user_id + " ) \n";
				text += "Key lautet -> ( " + success_response.data.auth_key + " )";
				
				worker.port.emit("USI-BACKEND:get-alert",text );
			
				// USER ist nun registriert
				worker.port.emit("USI-BACKEND:sync-is-user-registered-done", true);
				
			}, function (error_response){
				// Fehler aufgetreten
				worker.port.emit("USI-BACKEND:get-alert", "Benutzer konnte nicht erstellt werden" );
			});
		...
		worker.port.on("USI-BACKEND:syntax-error-test", function(userscipt_id){
            // hole das Userscript
			var userscript_handle = userscript_storage.getById(userscipt_id),
            userscript_content = page_injection_helper.createUserscriptContent(userscript_handle);
            
            // Aufruf der Funktion appendScript() in test.js
            var content_script = "appendScript(`" + userscript_content.replace(/\`/g, "\\\`") + "`);";
            
			var tabs = require("sdk/tabs");
            tabs.open({
                url: resource_path + "options/extra/syntax_test/test.html"
                , onReady: function (tab) {
                    tab.attach({
                        contentScriptFile: resource_path + "options/extra/syntax_test/test.js"
...
function convertToText(obj) {
	...
	//    arrays do.
	if (!basic_helper.isset(obj)) {
		return String(obj);
		//is null
	} else if (basic_helper.is_null(obj)) {
		string.push("NULL");
	} else if (basic_helper.is_obj(obj) && (obj.join === "undefined")) {
		for (var prop in obj) {
		...
		return "[" + string.join(",") + "]";
		//is function
	} else if (basic_helper.is_func(obj)) {
...
function responseToString(response, plain){
	var output = [];
	output.push("URL: " + response.url);
	output.push("STATUS-CODE: " + response.status);
	
	if(plain !== true){
		output.push("JSON: " + convertToText(response.json));
	}else{
		output.push("TEXT: " + response.text);
	}
