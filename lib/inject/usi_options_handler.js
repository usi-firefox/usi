...
load_resource = require("../load/load_resource").load_resource,
// Chrome Path
		...
		worker.port.on("USI-BACKEND:delete-reset-GM-Values-userscript", function (userscipt_id){
			...
			for (var i in addon_storage.storage){
                if(i === userscipt_id){
                    ...
                    addon_storage.storage[i].val_store = {};
			...
			for (var id in addon_storage.storage){
				...
				if(basic_helper.isset(addon_storage.storage[id].settings.updateURL)){
					...
					(function(actual_script_id, loaded_script_version){
						...
						load_resource.load_userscript_by_url(addon_storage.storage[actual_script_id].settings.updateURL, function (userscipt_text){
