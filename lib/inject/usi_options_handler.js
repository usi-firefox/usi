...
//		,baseurl: chrome_path
	...
	,onAttach: function (worker) {
		...
		worker.port.on("USI-BACKEND:loadexternal-script_url", function (script_content_response) {
			...
			load_resource.load_userscript_by_url(script_content_response.script_url,
				function (user_script_text) {
					...
					var check_if_userscript_is_added = add_userscript.check_for_userscript_settings_and_save_it(user_script_text, worker, null, null, script_content_response.moreinformations);
		...
		worker.port.on("USI-BACKEND:new-usi-script_content", function (script_content_response) {
			...
			// Hier wird das UserScript weiterverarbeitet und gespeichert
			var check_if_userscript_is_added = add_userscript.check_for_userscript_settings_and_save_it(script_content_response.script, worker);
		...
		worker.port.on("USI-BACKEND:override-same-userscript", function (userscipt_infos){
			...
			// Hier wird das UserScript weiterverarbeitet und gespeichert
			var check_if_userscript_is_added = add_userscript.check_for_userscript_settings_and_save_it(userscipt_infos.userscript, worker, userscipt_infos.id, null, userscipt_infos.moreinformations);
