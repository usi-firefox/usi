...
page_injection.PageMod({
	...
	,contentScriptFile: [resource_path	+	"libs/jquery-2.2.1.min.js"	// always load jQuery
	...
	// CSS ausgelagert
	,contentStyleFile: [resource_path + "options/css/extra.css"].concat(angular_options.getAngularJsMobileCSSFiles())
	...
	,onAttach: function (worker) {
		...
		worker.port.emit("USI-BACKEND:storage-quota", addon_storage.quotaUsage);
                ...
                worker.port.on("USI-BACKEND:get-all-userscripts-for-export", function(complete_export){
                    ...
                    var infos = ["USI-EXPORT","VERSION:0.1","DATE:" + export_date,"COMPLETE:" + complete_export];
                    // infos hinzufügen
                    for (var i in infos){
                    ...
                    for (var i in addon_storage.storage){
                        if(complete_export === "FALSE"){
                            result_export += addon_storage.storage[i].userscript;
                            result_export += "\n" + separator;
                        }else{
                            result_export += JSON.stringify(addon_storage.storage[i]);
                        }
                    }
