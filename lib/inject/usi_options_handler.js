...
page_injection.PageMod({
	...
	,contentScriptOptions: {
		...
		,cssFiles: [resource_path + "options/css/extra.css", resource_path + "libs/pleasewait/please-wait.css"].concat(angular_options.getAngularJsMobileCSSFiles())
	}
	...
	,onAttach: function (worker) {
		...
		worker.port.emit("USI-BACKEND:OldUsiIncludeBehavior", preferences.prefs.OldUsiIncludeBehavior);
		worker.port.on("USI-BACKEND:OldUsiIncludeBehavior-change", function(state){
			preferences.prefs.OldUsiIncludeBehavior = state;
		});
		
		// Übermittelt den aktuellen Status ob .user.js Dateien importiert werden sollen
		worker.port.emit("USI-BACKEND:ExternalScriptLoadQuestion", preferences.prefs.enableExternalScriptLoadQuestion);
		worker.port.on("USI-BACKEND:ExternalScriptLoadQuestion-change", function(state){
			preferences.prefs.enableExternalScriptLoadQuestion = state;
		});
		
		// Übermittelt den aktuellen Status ob HighlightJS aktiviert ist oder nicht
		worker.port.emit("USI-BACKEND:highlightjs-activation-state", preferences.prefs.options_activate_highlightjs);
		worker.port.on("USI-BACKEND:highlightjs-activation-state-change", function(state){
			preferences.prefs.options_activate_highlightjs = state;
			worker.port.emit("USI-BACKEND:highlightjs-activation-state", preferences.prefs.options_activate_highlightjs);
		});
