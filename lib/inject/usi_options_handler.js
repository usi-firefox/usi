...
//						,resource_path	+	"libs/jquery/jquery-migrate-3.0.0.min.js"		// always load jQuery
//						resource_path	+	"libs/pleasewait/please-wait.min.js"	// PleaseWait.js
...
//						,resource_path	+	"libs/jquery.loadtemplate/jquery.loadTemplate.min.js"		// jQuery loadTemplate
						,resource_path	+	"helper/basic_helper.js"
						,resource_path	+	"libs/gentelella/js/bootstrap.min.js"	// Bootstrap Theme
						,resource_path	+	"libs/gentelella/js/custom.js"			// Bootstrap Theme
						,resource_path	+	"libs/fastclick/fastclick-1.0.6.min.js"	// FastClick um ng-click auf Touchscreens ohne delay zu nutzen / seit AngularJS 1.5.5
						,resource_path	+	"libs/highlight/highlight.pack.js"
						,resource_path	+	"libs/switchery/switchery.min.js"
//						,resource_path	+	"options/app/options.js"
						,resource_path	+	"options/js/controller/createDownload.js"
						...
						,resource_path	+	"options/js/controller/switchery.js"
...
//						,resource_path	+	"options/app/fastclick_init.js"
						]
		// Angular Module!
		.concat(
...
//		,[resource_path + "options/js/angular_functions.js", resource_path + "options/js/fastclick_init.js"]
				)
	...
	,contentScriptOptions: {
		...
		,cssFiles: [
			resource_path + "libs/gentelella/css/bootstrap.min.css"
			,resource_path + "libs/gentelella/css/custom.min.css"
			,resource_path + "libs/gentelella/css/font-awesome.min.css"
//			,resource_path + "options/css/extra.css"
			,resource_path + "libs/pleasewait/please-wait.css"
			,resource_path + "libs/switchery/switchery.min.css"
		]
	...
	,onAttach: function (worker) {
		...
		worker.port.on("USI-BACKEND:loadexternal-script_url", function (script_content_response) {
			...
			page_injection_helper.load_userscript_by_url(script_content_response.script_url,
				function (user_script_text) {
					...
					var check_if_userscript_is_added = page_injection_helper.check_for_userscript_settings_and_save_it(user_script_text, worker, null, null, script_content_response.moreinformations);
		...
		function changeState (event_name, pref_name){
			...
			worker.port.on("USI-BACKEND:" + event_name + "-change", function(state){
				...
				// Sende die Einstellung erneut zur Options Übersicht
				worker.port.emit("USI-BACKEND:" + event_name , preferences.prefs[pref_name]);
			});
		}
		
		// Übermittelt den aktuellen Status ob Greasemonkey Funktionen immer aktiviert sind oder nicht
		changeState("options_always_activate_greasemonkey", "options_always_activate_greasemonkey");
		
		// Übermittelt den aktuellen Status wie die Include Regeln verarbeitet werden sollen
		changeState("OldUsiIncludeBehavior", "OldUsiIncludeBehavior");
		
		// Übermittelt den aktuellen Status ob .user.js Dateien importiert werden sollen
		changeState("ExternalScriptLoadQuestion", "enableExternalScriptLoadQuestion");
		
		// Übermittelt den aktuellen Status ob HighlightJS aktiviert ist oder nicht
		changeState("highlightjs-activation-state", "options_activate_highlightjs");
		
		// Highlightjs Style festlegen
		changeState("highlightjs-style", "hightlightjsstyle");
		...
		worker.port.on("USI-BACKEND:override-same-userscript", function (userscipt_infos){
			...
			// Hier wird das UserScript weiterverarbeitet und gespeichert
			page_injection_helper.check_for_userscript_settings_and_save_it(userscipt_infos.userscript, worker, userscipt_infos.id, null, userscipt_infos.moreinformations);
		...
		worker.port.on("USI-BACKEND:delete-script-by-id", function (key) {
			...
			if (!basic_helper.empty(key) && basic_helper.isset(addon_storage.storage[key])) {
