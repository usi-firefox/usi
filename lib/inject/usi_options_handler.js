...
// Chrome Path
    chrome_path = addon_path_helper.chrome_path();
...
//		,baseurl: chrome_path
	...
	,onAttach: function (worker) {
		...
		worker.port.on("USI-BACKEND:syntax-error-test", function(userscript_id){
            // hole das Userscript
		...
		function getChangeStateEvents(){
			return [
				...
				,{
					// Eigenes CSS festlegen
					name: "config_add_css"
					,pref_name: "config_add_css"
				}
			];
		}
		...
		worker.port.on("USI-BACKEND:override-same-userscript", function (userscript_infos){
			
			// Hier wird das UserScript weiterverarbeitet und gespeichert
			add_userscript.check_for_userscript_settings_and_save_it(userscript_infos.userscript, worker, userscript_infos.id, null, userscript_infos.moreinformations,
			function(userscript_handle){
				// das Userscript direkt aktivieren
				page_injection_helper.init_page_injection_by_id(userscript_handle.getId());
			});
		...
		worker.port.on("USI-BACKEND:delete-reset-GM-Values-userscript", function (userscript_id){
			var userscript_handle = userscript_storage.getById(userscript_id);
			...
			for (var i in all_userscripts_handler){
				...
				(function(userscript_handler){
					...
					if(basic_helper.isset(userscript_handler.getSettings()["updateURL"])){
						...
						load_resource.load_userscript_by_url(userscript_handler.getSettings()["updateURL"], function (loaded_userscript_text){

							// Konfig suchen und danach die Optionen Parsen...
							var loaded_userscript_settings = parse_userscript.find_settings(loaded_userscript_text);
							...
							if(loaded_userscript_settings.version !== userscript_handler.getSettings()["version"]){
								...
								worker.port.emit("USI-BACKEND:update-for-userscript-available", {id: userscript_handler.getId(), userscript: loaded_userscript_text});
