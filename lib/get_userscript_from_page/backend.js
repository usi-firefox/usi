            ...
            switch (message.name) {
                case "USI-BACKEND:new-userscript":
                    ...
                    if (valid_userscript.valid === false) {
                        ...
                        basic_helper.notify(lang.getMessage("userscript_config_is_wrong"));
                        return;
                    }
                    ...
                    if (userscript_id === false) {
                        ...
                        basic_helper.notify(lang.getMessage("userscript_was_created"));
                ...
                case "USI-BACKEND:override-same-userscript":
                    ...
                    if (!basic_helper.empty(message.data.id) && !basic_helper.empty(message.data.userscript)) {
                        ...
                        let userscript_handle = await add_userscript.update_userscript(message.data.id, message.data.userscript, more_informations);
                        // füge das Skript gleich hinzu, damit es ausgeführt werden kann
                        page_injection_helper.add_userscript(userscript_handle);

                        basic_helper.notify(lang.getMessage("userscript_was_overwritten"));
                    }
                    break;
            }
        } catch (ex) {
