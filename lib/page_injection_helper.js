 ...
 */
 option_functions : function (worker) {
		...
		worker.port.on("loadexternal-script_url", function (script_content_response) {
...
//Angular
var angular_options = require("./angular_options").angular_options;


// Einstellungen!
page_injection.PageMod({
	include: cfx_jpm_helper.resource_path() + "options/options.html"
	,contentScriptWhen: 'end'
	,attachTo: ['existing', 'top'] // TODO FF: make blocking policy start before this is run on install
	,contentScriptFile: [
		cfx_jpm_helper.resource_path() + "external/jquery-2.1.4.min.js" // always load jQuery
		,cfx_jpm_helper.resource_path() + "helper/basic_helper.js"		// Helper Funktionen
		,cfx_jpm_helper.resource_path() + "options/options_content.js"
		// Angular Module!
	].concat(angular_options.getAngularJsJSFiles()
	,angular_options.getAngularJsMobileJSFiles()
	,[cfx_jpm_helper.resource_path() + "options/angular_functions.js"])
	
	// Stelle eine Möglichkeit zum übersetzen zur Verfügung
	,contentScriptOptions: {language: lang, init_storage_data: addon_storage.storage, version : self.version}
	
	// CSS ausgelagert
//	,contentStyleFile: [cfx_jpm_helper.resource_path() + "options/options.css"]
	,contentStyleFile: [cfx_jpm_helper.resource_path() + "options/options.css"].concat(angular_options.getAngularJsMobileCSSFiles())
...
// Horche auf User Scripte die aufgerufen wurden!
	page_injection.PageMod({
		...
		contentScriptWhen: 'end',
		attachTo: ['existing', 'top'], // TODO FF: make blocking policy start before this is run on install
		contentScriptFile: cfx_jpm_helper.resource_path() + "userscript-get.js",
		...
		onAttach: function (worker) {
			...
			worker.port.emit("active", preferences.prefs.enableExternalScriptLoadQuestion);
...
//                                contentScriptOptions: userscript_entry.options || {} 
		...
		if (script_settings["include-jquery"] == "true") {
			page_injection_object.contentScriptFile.push( cfx_jpm_helper.resource_path() + "external/jquery-2.1.4.min.js");
		}
		...
		if (script_settings["use-greasemonkey"] == "true") {
			...
			page_injection_object.contentScriptFile.push( cfx_jpm_helper.resource_path() + "helper/greasemonkey_functions_to_self_port.js");
			...
			page_injection_object.onAttach = function (worker) {
				...
				// Variable speichern
				worker.port.on("GM_setValue", function (value_pair) {
					grease_handler.GM_setValue(value_pair.name, value_pair.value);
				}); 
				 
				// Variable löschen
				worker.port.on("GM_deleteValue", function (value_pair) {
					grease_handler.GM_deleteValue(value_pair.name);
				});

				// neuen Tab öffnen
				worker.port.on("GM_openInTab", function (value_pair) {
					...
					if(not_wildcard_pagemod === true){
						try {
							require("sdk/tabs").open(value_pair.url);
						} catch (ex) {}
					}
				});
