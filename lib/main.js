...
// Initialisierung für das Native Window (Fennec)
	...
	if (system.platform === "android") {
        ...
        // Extra Button für den Desktop
        var usi_configure_button = require("sdk/ui").ActionButton({
            id: "UsiConfigureButton",
            label: _("usi_options"),
            icon: "./icon/usi.png",
            onClick: function () {
                openUsiOptions();
...
//************* MIGRATIONS BEREICH für die Webextension *************//
const webext = require("sdk/webextension");
const {setSyncLegacyDataPort} = require("../migrate/sdk-data-storage");

webext.startup().then(({browser}) => {
    browser.runtime.onConnect.addListener(port => {
        if (port.name === "usi-sdk-addon-data") {
