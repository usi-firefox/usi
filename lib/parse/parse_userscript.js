...
parse_userscript = {
	...
	add_option_to_userscript_metablock : function(userscript, new_entries){
		...
		var userscript_metablock	= this.find_lines_with_settings(userscript);
		var userscript_rest			= this.find_lines_with_settings(userscript, true);
		
		var new_userscript_metablock	= this.create_metablock_from_array(userscript_metablock, new_entries);
	...
	/**
	 * ersetzt ".tld" durch => \.[a-zA-Z0-9\-\.]*
	 * @param {string} url
	 * @returns {string}
	 */
	replace_tld_in_url : function (url){
		var result_url = url;
		
		if(typeof url !== "undefined" && /\.tld/.test(url)){
			result_url = url.replace(/\.tld/, "\.[a-zA-Z0-9\\-\\.]*");
		}
	...
	/**
	 * Zählt die Vorkommen der Wildcards
	 * @param {string} str 
	 * @returns {number}
	 */
	count_wildcard_in_str : function (str){
		var wildcards_count = 0;
		if (typeof str === "string") {
			// Zählen der wildcards
			var wildcards_count_tmp = str.match(/\*/g);

			if (wildcards_count_tmp !== null && typeof wildcards_count_tmp.length === "number") {
				wildcards_count = wildcards_count_tmp.length;
			}
		}
		return wildcards_count;
	},

	/**
	 * Prüft ob der String als normale Domain gewertet werden soll
	 * regex_test_wildcard_first_position_without_trailing_slash
	 * 
	 * @param {string} str
	 * @returns {Boolean}
	 */
	regex_test_only_domain : function (str){
		var result = /(\*?([a-zA-Z0-9\-\.]+))(?!\/)/.exec(str);
		
		// Wenn result[0] mit str identisch ist, ist alles in Ordnung
	...
	/**
	 * Testet ob ein Schema angegeben wurde
	 * @param {string} str
	 * @returns {Boolean}
	 */
	regex_test_schema : function (str){
		if(/^(https?|ftp|resource|file|data):\/\//.test(str)){
			return true;
		}else{
			return false;
		}
...
//			if(typeof rule.trim === "function"){
//				rule = rule.trim();
//			}
//			
//			// Wildcards zählen
//			var wildcards_count = this.count_wildcard_in_str(rule);
//			
//			// Anpassung der rule für Pagemod
//			// -> https://developer.mozilla.org/en-US/Add-ons/SDK/Low-Level_APIs/util_match-pattern
//			switch(rule){
//				/**
//				 * gilt für alle Webseiten
//				 * -> (ftp|http|https)://.*
//				 */
//				case "*":
...
//					
//				default:
//					// Verarbeitung durch GM_convert2RegExp
//					return GM_convert2RegExp(rule);
//					
//				/**
//				 * es ist ein regex, führe keine Anpassungen durch
//				 */
//				case (/^\/.*\/$/.test(rule)):
//					return new RegExp(rule);
//			
//				/**
//				 * Kein bekanntes Schema, daher wird es als ganze Domain gewertet
//				 * wenn lediglich 0 oder 1 * verwendet wird (nur an erster Stelle erlaubt!)
//				 * Beispiel: www.mozilla.org oder *.foo.org oder *.example.com
//				 */
//				case ( !(this.regex_test_schema(rule)) && 
//							wildcards_count <= 1 && 
//							this.regex_test_only_domain(rule)):
//					var test = GM_convert2RegExp(rule);
//							console.error(test);
//							console.error(rule);
//					return GM_convert2RegExp(rule);
//				
//				/**
//				 * Normale Regel inklusive Schema
//				 * und wenn lediglich 0 oder 1 * verwendet wird
//				 * Beispiel: https://www.mozilla.org/ oder https://www.example.*
//				 */
//				case (this.regex_test_schema(rule) && wildcards_count <= 1):
//					return GM_convert2RegExp(rule);
//
//				/**
//				 * Mehr als ein Wildcard gefunden
//				 * daher ist eine Umwandlung in einen Regex nötig
//				 */
//				case (wildcards_count > 1):
//					return GM_convert2RegExp(rule);
//			}
//						
//		}else{
