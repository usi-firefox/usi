...
parse_userscript = {
	...
	find_settings: function (userscript) {
		...
		var possible_entries = [
			{multiple: false,	val: "name"},
			{multiple: false,	val: "namespace"},
			{multiple: false,	val: "author"},
			{multiple: false,	val: "homepageURL"},
			{multiple: false,	val: "date"},
			{multiple: false,	val: "license"},
			{multiple: false,	val: "icon"},
			{multiple: false,	val: "description"},
			{multiple: true,	val: "exclude"},
			{multiple: true,	val: "match"},			// Match und Include werden gleich behandelt!
			{multiple: true,	val: "include"},		// Match und Include werden gleich behandelt!
			{multiple: false,	val: "clean-include"},	// verhindert das Extra Parsing der Includes durch USI
			{multiple: false,	val: "info"},
			{multiple: true,	val: "resource"},		// Damit du andere Dinge zusätzlich herunterladen kannst
			{multiple: false,	val: "run-at"},			// document-end || document-start || document-ready
			{multiple: true,	val: "grant"},
			{multiple: false,	val: "include-jquery"},	// true || false --- du brauchst zusätzlich jQuery? setze "true" || "false", keine sorge es wird vorher geprüft ob bereits jQuery auf der Seite existiert!
			{multiple: false,	val: "updateURL"},		// Hierüber wird später geprüft ob eine neue Version vom Skript zur Verfügung steht
			{multiple: false,	val: "downloadURL"},		// Hierüber wird später geprüft ob eine neue Version vom Skript zur Verfügung steht
			{multiple: false,	val: "version"},
			{multiple: false,	val: "use-greasemonkey"}, // true || false
			{multiple: true,	val: "options"}			// Damit kannst du mehrere Werte bestimmen, die dein UserScript nutzen soll!
		];
		...
		for (var i in possible_entries) {
			...
			multiple = possible_entries[i].multiple,
